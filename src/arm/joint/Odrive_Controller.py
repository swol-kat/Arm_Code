class Odrive_Controller:
    ...
    def send_packet():
        ...
        if self.state = 'set_params':
            ...
            joint1.configure()
            odrive_command = {}
            odrive_command['axis_0'] = joint0.get_command()
            odrive_command['axis_1'] = joint1.get_command()
            odrive_command['command'] = set_odrive_params()
            odrive_command['index'] = next_index
            next_index = next_index + 1
            self.state = 'wait_for_response'
            odrive_pipe.send(odrive_command)
        ...
        if self.state == 'wait_for_response':
            #you done fucked up if here
            return

    
    def set_odrive_params():
        self.state = 'set_params'
    
    def block_for_response():
        if self.state != 'wait_for_response':
            #catch command not sent so infinite block
            return
        index = 0
        data = {}
        while (index + 1) != next_index:
            data_in = self.odrive_pipe.recv()
            index = data_in['index']

        joint0.recieve_data(data_in['axis_0'])
        joint1.recieve_data(data_in['axis_1'])
        self.state = 'ready'
